...
contract GameFacet2 is Modifiers {
    ...
    function checkWinner(uint256 matchId) internal {
        ...
        if (player2Points < 4) {
            IPool(s.aavePool).withdraw(s.dai, winnerPrize, s.matches[matchId].player1);
            s.matches[matchId].winner = s.matches[matchId].player1;
        } else if (player2Points > 4) {
            IPool(s.aavePool).withdraw(s.dai, winnerPrize, s.matches[matchId].player2);
            s.matches[matchId].winner = s.matches[matchId].player2;
        } else if (player2Points == 4 ) {
            IPool(s.aavePool).withdraw(s.dai, s.matches[matchId].betsize * 1 ether, s.matches[matchId].player1);
            IPool(s.aavePool).withdraw(s.dai, s.matches[matchId].betsize * 1 ether, s.matches[matchId].player2);
        }
    ...
    function contestMatch(uint256 matchId) external {
        ...
        require(
            msg.sender == s.matches[matchId].player1 || 
            msg.sender == s.matches[matchId].player2 || 
            msg.sender == LibDiamond.contractOwner(), "GameFacet2: you are not allowed"
        );
        uint256 winnerPrize = s.matches[matchId].betsize * 2 ether;
